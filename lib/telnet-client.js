// tell jshint to ignore missing semicolon
/* jshint -W033 */
...
Telnet.prototype.connect = function(opts) {
  ...
  return new Promise(function(resolve) {
    ...
    self.pageSeparator = (typeof opts.pageSeparator !== 'undefined' ? opts.pageSeparator : '---- More')
    self.response = ''
    // self.telnetState
...
function parseData(chunk, telnetObj, callback) {
  ...
  if (telnetObj.debug) {
    console.log('==== Telnet Library - Current state: ', telnetObj.telnetState, '  ====');
    console.log('==== Telnet Library - Current prompt ', telnetObj.shellPrompt, '  ====');
    console.log('==== Telnet Library - String data in this chunk\n', stringData, '\n========');
  }

  if (telnetObj.telnetState === 'getprompt') {
    promptIndex = stringData.search(telnetObj.shellPrompt)
    if (promptIndex !== -1) {
      var rxdShellPrompt = stringData.substring(promptIndex)
      telnetObj.telnetState = 'sendcmd'
      telnetObj.stringData = ''
      telnetObj.emit('ready', rxdShellPrompt)

      if (callback) callback('ready', rxdShellPrompt)
    }
    ...
    if (telnetObj.debug) console.log('==== Telnet Library - current string data in object\n', telnetObj.stringData, '\n===========');
    // convert back to regex if it has been changed.
    telnetObj.shellPrompt = checkRegExp(telnetObj.shellPrompt);
    if (!telnetObj.shellPrompt.test(telnetObj.stringData) && stringData.length !== 0) {
...
function checkRegExp(str, def){
  ...
  if (str instanceof RegExp) {
    ...
    // and need to escape important regex characters
    try {
